...
hit_result_type bvh_node::hit(const ray &r, double t_min, double t_max) const {
...
bound_result_type bvh_node::bounding_box(double, double) const {
    return std::make_optional<aabb>(box_);
}

hittable_list bvh(const hittable_list &list, double a, double b) {
    hittable_list world;
